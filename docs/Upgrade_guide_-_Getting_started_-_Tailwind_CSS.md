...
tailwindcss
```
 package was a PostCSS plugin, but in v4 the PostCSS plugin lives in a dedicated 
```
@tailwindcss/postcss
```
 package.
...
Tailwind CSS v4.0 is designed for modern browsers and targets Safari 16.4, Chrome 111, and Firefox 128. We depend on modern CSS features like 
```
@property
```
 and 
...
color-mix()
```
 for core framework features, and Tailwind CSS v4.0 will not work in older browsers.
...
We've renamed the default shadow, radius and blur scales to make sure every utility has a named value. The "bare" versions still work for backward compatibility, but the 
```
_<utility>_-sm
```
 utilities will look different unless updated to their respective 
```
_<utility>_-xs
```
 versions.
...
outline-width: 1px
```
 by default to be more consistent with border and ring utilities. Furthermore all 
```
outline-<number>
```
 utilities default 
...
solid
```
, omitting the need to combine them with 
```
outline
```
:
...
outline-none
```
 utility that actually sets 
```
outline-style: none
```
.

To update your project for this change, replace any usage of 
```
outline-none
```
 with 
...
3px
```
 ring. We've changed this in v4 to be 
```
1px
```
...
We've changed the selector used by the [
```
space-x-*
```
 and 
...
space-y-*
```
 utilities](https://tailwindcss.com/docs/margin#adding-space-between-children) to address serious performance issues on large pages:
...
If this change causes any issues in your project, we recommend migrating to a flex or grid layout and using 
```
gap
```
 instead:
...
In v3, overriding part of a gradient with a variant would "reset" the entire gradient, so in this example the 
```
to-*
```
...
This means you may need to explicitly use 
```
via-none
```
...
container
```
 utility had several configuration options like 
```
center
```
 and 
...
container
```
 utility in v4, extend it using the 
```
@utility
```
 directive:
...
divide-*
```
 utilities used your configured 
```
gray-200
```
 color by default. We've changed this to 
```
currentColor
```
 in v4 to make Tailwind less opinionated and match browser defaults.

To update your project for this change, make sure you specify a color anywhere you're using a 
```
border-*
```
 or 
...
We've changed the width of the 
```
ring
```
 utility from 3px to 1px and changed the default color from 
```
blue-500
```
 to 
...
To update your project for these changes, replace any use of 
```
ring
```
 with 
...
In v3, placeholder text used your configured 
```
gray-400
```
...
flex
```
 no longer take priority over the 
```
hidden
```
 attribute on an element. Remove the 
```
hidden
```
 attribute if you want an element to be visible to the user. Note that this does not apply to 
```
hidden="until-found"
```
.
...
In v3, any custom classes you defined within 
```
@layer utilities
```
 or 
...
@layer components
```
 would get picked up by Tailwind as a true utility class and would automatically work with variants like 
```
hover
```
, 
...
In v4 we are using native cascade layers and no longer hijacking the 
```
@layer
```
 at-rule, so we've introduced the 
```
@utility
```
...
Custom utilities are now also sorted based on the amount of properties they define. This means that component utilities like this 
```
.btn
```
...
*
```
) and any typography plugin variants (
```
prose-headings
```
...
In v3 you were able to use CSS variables as arbitrary values without 
```
var()
```
...
Commas were previously replaced with spaces in the 
```
grid-cols-*
```
, 
...
object-*
```
 utilities inside arbitrary values. This special behavior existed in Tailwind CSS v3 for compatibility with v2. This compatibility no longer exists in v4.0 and underscores must be used to represent spaces.
...
This can create problems if you've built your site in a way that depends on touch devices triggering hover on tap. If this is an issue for you, you can override the 
```
hover
```
...
Since v4 includes CSS variables for all of your theme values, we recommend using those variables instead of the 
```
theme()
```
...
For cases where you still need to use the 
```
theme()
```
...
If you still need to use a JavaScript config file, you can load it explicitly using the 
```
@config
```
 directive:
...
In v4, stylesheets that are bundled separately from your main CSS file (e.g. CSS modules files, 
```
<style>
```
...
To make these definitions available in these contexts, use [
```
@reference
```
](https://tailwindcss.com/docs/functions-and-directives#reference-directive) to import them without duplicating any CSS in your bundle:
...
Alternatively, you can use your CSS theme variables directly instead of using 
```
@apply
```
...
<style>
```
 blocks in Vue, Svelte, Astro, etc.
