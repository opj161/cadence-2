...
tailwindcss
```
 into your project, Preflight is automatically injected into the 
```
base
```
 layer:
...
In order to make it easy to add a border by simply adding the 
```
border
```
...
border-width
```
 property, this reset ensures that adding that class always adds a solid 
```
1px
```
 border that uses 
...
If you ever need to make one of these elements 
```
inline
```
 instead of 
...
This prevents them from overflowing their containers and makes them responsive by default. If you ever need to override this behavior, use the 
```
max-w-none
```
 utility:
...
hidden
```
 attribute stay hidden](#elements-with-a-hidden-attribute-stay-hidden)
...
This enforces that elements with a 
```
hidden
```
 attribute stay invisible unless using 
```
hidden="until-found"
```
. Remove the 
...
hidden
```
 attribute if you want an element to be visible to the user.

## [Extending Preflight](#extending-preflight)

If you'd like to add your own base styles on top of Preflight, add them to the 
```
base
```
 CSS layer in your CSS using 
```
@layer base
```
:
...
When importing Tailwind CSS' files individually, features like 
```
source()
```
, 
...
prefix()
```
 should go on their respective imports.

For example, source detection affects generated utilities, so 
```
source(…)
```
...
theme(inline)
```
 affect the generated theme variables and should be placed on the 
```
theme.css
```
 import:
...
Finally, using a prefix with 
```
prefix(tw)
```
 affects the utilities and variables, so it should go on both imports:
